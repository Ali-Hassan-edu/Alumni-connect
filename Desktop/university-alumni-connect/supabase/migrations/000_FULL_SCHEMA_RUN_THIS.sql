...
RETURNS TABLE(user_id UUID, full_name TEXT, skills TEXT[], match_count BIGINT) AS $$
BEGIN
  RETURN QUERY
    SELECT u.id, u.full_name, sp.skills,
      (SELECT COUNT(*) FROM unnest(sp.skills) s WHERE s = ANY(task_skills)) AS match_count
    FROM users u
