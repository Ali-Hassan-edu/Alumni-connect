...
import { notificationQueries, userQueries } from '@/lib/supabase/queries'
...
export default function NewTaskPage() {
  ...
  const onSubmit = async (data: TaskFormData) => {
    ...
    try {
      ...
      toast.success('Task posted! Admin will review and assign suitable students.')
    ...
    <DashboardLayout>
      <div className="p-6 lg:p-8 max-w-3xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <Link to="/tasks" className="p-2 rounded-xl border border-border hover:bg-accent transition-colors">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Post New Task</h1>
            <p className="text-muted-foreground text-sm">Admin will assign suitable students based on skills</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
            ...
            <div>
              ...
              <textarea {...register('description')} rows={5} placeholder="Describe the task in detail. Include requirements, deliverables, and any specific expectations..." className={`${inputClass} resize-none`} />
          ...
          {/* Settings */}
          <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
            <h2 className="font-semibold text-gray-900 dark:text-white">Task Settings</h2>
          ...
          <div className="flex items-center justify-end gap-3">
            <Link to="/tasks" className="px-5 py-2.5 border border-border rounded-xl text-sm font-medium hover:bg-accent transition-colors">Cancel</Link>
