  ...
  return (
    <Link to={`/tasks/${task.id}`} className="block p-5 bg-card border border-border rounded-2xl hover:border-blue-200 dark:hover:border-blue-800/50 hover:shadow-md transition-all">
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground pt-3 border-t border-border">
        ...
        <span className={`px-2 py-0.5 rounded-full font-medium text-xs ${STATUS_CONFIG[task.status] || 'bg-gray-100 text-gray-600'}`}>{task.status.replace('_', ' ')}</span>
      </div>
    </Link>
  )
}
...
export default function TasksPage() {
  ...
  const loadTasks = useCallback(async () => {
    ...
    setIsLoading(false)
  }, [dbUser?.id, myTasks, filterStatus, filterPriority, search])
      ...
      <div className="p-6 lg:p-8">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 bg-card border border-border rounded-2xl">
            ...
            <p className="text-muted-foreground text-sm mb-4">{myTasks ? "You haven't posted any tasks yet." : "No tasks match your filters."}</p>
